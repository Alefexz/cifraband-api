...
function scheduleMutation(data, profile, uid, body, now) {
    ...
    if (body.action === 'suggest') {
        ...
        if (!song.title || !song.artist) fail('Informe nome e artista.');
        if (input.kind === 'listening') {
            if (song.title.length > 300 || song.artist.length > 300) fail('Nome ou artista muito longo.');
            if (song.referenceUrl?.trim()) {
                let link;
                try { link = new URL(song.referenceUrl.trim()); } catch (_) { fail('Link invalido.'); }
                const hosts = new Set(['youtube.com', 'www.youtube.com', 'm.youtube.com', 'music.youtube.com', 'youtu.be',
                    'open.spotify.com', 'spotify.link', 'deezer.com', 'www.deezer.com', 'link.deezer.com',
                    'music.apple.com', 'soundcloud.com', 'www.soundcloud.com', 'on.soundcloud.com']);
                if (link.protocol !== 'https:' || link.username || link.password || link.port ||
                    !hosts.has(link.hostname) || link.pathname === '/') fail('Link de musica invalido.');
                song.referenceUrl = link.href;
            }
            // Listening suggestions must never become fake chord sheets/keys.
            for (const field of ['key', 'capo', 'originalKey', 'shapeKey', 'content', 'url', 'bpm']) delete song[field];
            song.kind = 'listening';
        } else if (input.kind != null && input.kind !== 'chord') {
            fail('Tipo de sugestao invalido.');
        } else if (!song.content?.trim()) fail('Cifra incompleta.');
...
function mountMemberActions(app, { authenticate, limit, getAdmin, songLinks }) {
    ...
    route('/members/song-links', async (db, uid, body) => {
        if (!validId(body.scheduleId) || !['youtube', 'spotify'].includes(body.platform) ||
            typeof body.title !== 'string' || typeof body.artist !== 'string' ||
            !body.title.trim() || !body.artist.trim() || body.title.length > 300 || body.artist.length > 300) fail('Musica invalida.');
        const profile = (await db.collection('users').doc(uid).get()).data();
        const schedule = (await db.collection('schedules').doc(body.scheduleId).get()).data();
        if (!profile?.church_id || profile.church_id !== schedule?.church_id) fail('Sem acesso a esta escala.', 403);
        const songs = [...(schedule.suggested_songs || []), ...(schedule.approved_songs || [])];
        const song = songs.find(s => s.title === body.title && s.artist === body.artist);
        if (!song) fail('Sugestao nao encontrada.', 404);
        if (!songLinks) fail('Servico indisponivel.', 503);
        return songLinks.resolve(body.platform, song.title, song.artist);
    });

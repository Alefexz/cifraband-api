...
function isEligible(device, version) {
    return device.platform === 'android' && device.distribution !== 'play' && device.enabled !== false &&
...
function parseRegistration(body) {
    ...
    if (body.distribution != null && !['play', 'direct'].includes(body.distribution)) return null;
    return { token: body.token, build: body.build, platform: 'android',
        ...(body.distribution ? {distribution: body.distribution} : {}) };
}
